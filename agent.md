...
- 用模块化文档管理规则，降低实现偏差。
- 让开发、测试、评审都能按同一套契约执行。
- 明确产品目标用户为不懂代码的普通人，交互设计必须优先保障零代码可用性。
...
- 涉及 Renderer 视觉改动时，必须保证预览与导出复用同一份样式源，并验证两者一致性，禁止双份样式并行维护。
- 每次修改必须提交 commit，禁止长期保留未提交变更。
- 修改 CI/CD 或部署流程时，必须同步更新 `README.md`（包含触发条件与部署步骤）并在 `docx/10-change-log.md` 记录。
