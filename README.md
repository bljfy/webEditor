...
- 状态可视化：编辑面板顶部显示“未保存/已保存/校验失败”状态
- 双模式编辑：简洁模式（默认）与高级模式（显示低频配置）
- 任务流分组：基础信息 / 首页内容 / 内容区块 / 页脚与导出
...
- 字段级错误定位：关键输入项显示就地错误提示
- 双滚动布局：`Panel` 常驻可见，`Preview` 独立滚动
- 移动端编辑/预览切换：小屏下通过顶部 Tab 切换视图
- 移动端快速保存：未保存时显示悬浮“快速保存”按钮
